...
platform = __toESM(platform);
let node_crypto = require("node:crypto");
node_crypto = __toESM(node_crypto);
let _connectrpc_connect = require("@connectrpc/connect");
let compare_versions = require("compare-versions");
let _bufbuild_protobuf_codegenv2 = require("@bufbuild/protobuf/codegenv2");
let _bufbuild_protobuf_wkt = require("@bufbuild/protobuf/wkt");
let _connectrpc_connect_web = require("@connectrpc/connect-web");
...
let dockerfile_ast = require("dockerfile-ast");
//#region package.json
var version = "2.33.1";
//#endregion
//#region src/utils.ts
function getRuntime() {
	var _process, _navigator;
	...
	if (((_process = globalThis.process) === null || _process === void 0 || (_process = _process.release) === null || _process === void 0 ? void 0 : _process.name) === "node") return {
		runtime: "node",
		version: platform.default.version || "unknown"
	};
...
async function toUploadBody(data, gzip) {
	if (gzip) {
		const compressed = (data instanceof ReadableStream ? data : data instanceof Blob ? data.stream() : new Blob([data]).stream()).pipeThrough(new CompressionStream("gzip"));
		return runtime === "browser" ? new Response(compressed).blob() : compressed;
	}
	if (data instanceof ReadableStream && runtime !== "browser") return data;
	return toBlob(data);
}
//#endregion
...
const defaultHeaders = {
	browser: typeof window !== "undefined" && platform.default.name || "unknown",
	lang: "js",
	lang_version: runtimeVersion,
	package_version: version,
	publisher: "e2b",
	sdk_runtime: runtime,
	system: ((_platform$os = platform.default.os) === null || _platform$os === void 0 ? void 0 : _platform$os.family) || "unknown"
};
...
function limitConcurrency(fetcher, max) {
	...
	return (async (input, init) => {
		var _init$signal;
		const signal = (_init$signal = init === null || init === void 0 ? void 0 : init.signal) !== null && _init$signal !== void 0 ? _init$signal : input instanceof Request ? input.signal : void 0;
...
async function loadUndici() {
	try {
		return await new Function("moduleName", "return import(moduleName)")("undici");
	} catch (_unused) {
		return;
	}
}
...
function createApiFetchForRuntime(currentRuntime = runtime, options = {}) {
	if (currentRuntime !== "node") return fetch;
	let fetcherPromise;
	return (async (input, init) => {
		var _fetcherPromise;
		(_fetcherPromise = fetcherPromise) !== null && _fetcherPromise !== void 0 || (fetcherPromise = buildApiFetcher(options));
		return (await fetcherPromise)(input, init);
	});
}
async function buildApiFetcher(options) {
	var _options$loadUndici, _options$inflightLimi, _options$connectionLi;
	const undici = await ((_options$loadUndici = options.loadUndici) !== null && _options$loadUndici !== void 0 ? _options$loadUndici : loadUndici)();
	const inflightLimit = (_options$inflightLimi = options.inflightLimit) !== null && _options$inflightLimi !== void 0 ? _options$inflightLimi : getApiInflightLimit();
	if (!undici) return limitConcurrency(fetch, inflightLimit);
	const { Agent, ProxyAgent, fetch: undiciFetch } = undici;
	const connections = (_options$connectionLi = options.connectionLimit) !== null && _options$connectionLi !== void 0 ? _options$connectionLi : getApiConnectionLimit();
...
function handleApiError(response, errorClass = SandboxError, stackTrace) {
	...
	if (response.response.status === 401) {
		...
		const content = (_response$error$messa = (_response$error = response.error) === null || _response$error === void 0 ? void 0 : _response$error.message) !== null && _response$error$messa !== void 0 ? _response$error$messa : response.error;
		if (content) return new AuthenticationError(`${message} - ${content}`);
		return new AuthenticationError(message);
	}
	if (response.response.status === 429) {
		var _response$error$messa2, _response$error2;
		const message = "Rate limit exceeded, please try again later";
		const content = (_response$error$messa2 = (_response$error2 = response.error) === null || _response$error2 === void 0 ? void 0 : _response$error2.message) !== null && _response$error$messa2 !== void 0 ? _response$error$messa2 : response.error;
		if (content) return new RateLimitError(`${message} - ${content}`);
		return new RateLimitError(message);
	}
	const message = ((_response$error3 = response.error) === null || _response$error3 === void 0 ? void 0 : _response$error3.message) || response.error || response.response.statusText;
...
var ApiClient = class {
	constructor(config, opts = {}) {
		...
		this.api = (0, openapi_fetch.default)({
			baseUrl: config.apiUrl,
			fetch: createApiFetch(config.proxy),
			headers: _objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2({}, defaultHeaders), config.apiKey && { "X-API-KEY": config.apiKey }), config.accessToken && { Authorization: `Bearer ${config.accessToken}` }), config.headers),
...
function setupRequestController(requestTimeoutMs, userSignal) {
	...
	const onUserAbort = () => controller.abort(userSignal === null || userSignal === void 0 ? void 0 : userSignal.reason);
	if (userSignal) if (userSignal.aborted) controller.abort(userSignal.reason);
	else userSignal.addEventListener("abort", onUserAbort, { once: true });
	let reqTimeout = requestTimeoutMs ? setTimeout(() => controller.abort(new DOMException(`Request handshake timed out after ${requestTimeoutMs}ms`, "TimeoutError")), requestTimeoutMs) : void 0;
...
function createIdleAbort(controller, idleTimeoutMs, label) {
	...
	const arm = () => {
		...
		clear();
		timer = setTimeout(() => controller.abort(new DOMException(`${label} idle for ${idleTimeoutMs}ms`, "TimeoutError")), idleTimeoutMs);
	};
	return {
		arm,
		clear
...
var ConnectionConfig = class ConnectionConfig {
	...
	*/
	...
	constructor(opts) {
		...
		this.accessToken = (opts === null || opts === void 0 ? void 0 : opts.accessToken) || ConnectionConfig.accessToken;
...
function isConnectionTerminatedError(err) {
	return err instanceof _connectrpc_connect.ConnectError && err.code === _connectrpc_connect.Code.Unknown && isConnectionTerminatedMessage(err.rawMessage);
}
const DEFAULT_ERROR_MAP$1 = {
	[_connectrpc_connect.Code.InvalidArgument]: (message) => new InvalidArgumentError(message),
	[_connectrpc_connect.Code.Unauthenticated]: (message) => new AuthenticationError(message),
	[_connectrpc_connect.Code.NotFound]: (message) => new NotFoundError(message),
	[_connectrpc_connect.Code.ResourceExhausted]: (message) => new RateLimitError(`${message}: Rate limit exceeded, please try again later.`),
	[_connectrpc_connect.Code.Unavailable]: formatSandboxTimeoutError,
	[_connectrpc_connect.Code.Canceled]: (message) => new TimeoutError(`${message}: This error is likely due to exceeding 'requestTimeoutMs'. You can pass the request timeout value as an option when making the request.`),
	[_connectrpc_connect.Code.DeadlineExceeded]: (message) => new TimeoutError(`${message}: This error is likely due to exceeding 'timeoutMs' — the total time a long running request (like command execution or directory watch) can be active. It can be modified by passing 'timeoutMs' when making the request. Use '0' to disable the timeout.`)
};
/**
...
function handleRpcError(err, errorMap) {
	if (err instanceof _connectrpc_connect.ConnectError) {
...
function authenticationHeader(envdVersion, username) {
	if (username == void 0 && (0, compare_versions.compareVersions)(envdVersion, "0.4.0") < 0) username = defaultUsername;
	if (!username) return {};
...
async function handleProcessStartEvent(events) {
	...
	try {
		...
		if (err instanceof _connectrpc_connect.ConnectError) {
			if (err.code === _connectrpc_connect.Code.Unavailable) throw new SandboxNotFoundError("Sandbox is probably not running anymore");
		}
		throw err;
	}
...
//#endregion
//#region src/envd/filesystem/filesystem_pb.ts
/**
* Describes the file filesystem/filesystem.proto.
*/
const file_filesystem_filesystem = /*@__PURE__*/ (0, _bufbuild_protobuf_codegenv2.fileDesc)("", [_bufbuild_protobuf_wkt.file_google_protobuf_timestamp]);
/**
* @generated from service filesystem.Filesystem
*/
const Filesystem$1 = /*@__PURE__*/ (0, _bufbuild_protobuf_codegenv2.serviceDesc)(file_filesystem_filesystem, 0);
//#endregion
...
const FILESYSTEM_RPC_ERROR_MAP = { [_connectrpc_connect.Code.NotFound]: (message) => new FileNotFoundError(message) };
...
var Filesystem = class {
	constructor(transport, envdApi, connectionConfig) {
		...
		this.rpc = (0, _connectrpc_connect.createClient)(Filesystem$1, transport);
	...
	async read(path, opts) {
		...
		if (user == void 0 && (0, compare_versions.compareVersions)(_this.envdApi.version, "0.4.0") < 0) user = defaultUsername;
		const headers = {};
	...
	async write(pathOrFiles, dataOrOpts, opts) {
		...
		if (user == void 0 && (0, compare_versions.compareVersions)(_this2.envdApi.version, "0.4.0") < 0) user = defaultUsername;
		const useGzip = (writeOpts === null || writeOpts === void 0 ? void 0 : writeOpts.gzip) === true;
		const supportsOctetStream = (0, compare_versions.compareVersions)(_this2.envdApi.version, ENVD_OCTET_STREAM_UPLOAD) >= 0;
		...
		if (metadata && Object.keys(metadata).length > 0 && (0, compare_versions.compareVersions)(_this2.envdApi.version, "0.6.2") < 0) throw new TemplateError("File metadata requires envd 0.6.2 or later.");
		...
		if (useOctetStream) {
			...
			const uploadResults = await Promise.all(writeFiles.map(async (file) => {
				...
				const signal = isStream ? writeOpts === null || writeOpts === void 0 ? void 0 : writeOpts.signal : _this2.connectionConfig.getSignal(writeOpts === null || writeOpts === void 0 ? void 0 : writeOpts.requestTimeoutMs, writeOpts === null || writeOpts === void 0 ? void 0 : writeOpts.signal);
				const res = await _this2.envdApi.api.POST("/files", _objectSpread2({
					...
					signal,
					body: {}
				}, isStream && { duplex: "half" })).catch(async (err) => {
	...
	*/
	async makeDir(path, opts) {
		...
		try {
			...
			if (err instanceof _connectrpc_connect.ConnectError) {
				if (err.code === _connectrpc_connect.Code.AlreadyExists) return false;
			}
	...
	*/
	async exists(path, opts) {
		...
		try {
			...
			if (err instanceof _connectrpc_connect.ConnectError) {
				if (err.code === _connectrpc_connect.Code.NotFound) return false;
			}
	...
	*/
	async watchDir(path, onEvent, opts) {
		...
		if ((opts === null || opts === void 0 ? void 0 : opts.recursive) && _this10.envdApi.version && (0, compare_versions.compareVersions)(_this10.envdApi.version, "0.1.4") < 0) throw new TemplateError("You need to update the template to use recursive watching.");
		if ((opts === null || opts === void 0 ? void 0 : opts.includeEntry) && _this10.envdApi.version && (0, compare_versions.compareVersions)(_this10.envdApi.version, "0.6.3") < 0) throw new TemplateError("You need to update the template to include entry info in watch events.");
		if ((opts === null || opts === void 0 ? void 0 : opts.allowNetworkMounts) && _this10.envdApi.version && (0, compare_versions.compareVersions)(_this10.envdApi.version, "0.6.4") < 0) throw new TemplateError("You need to update the template to watch directories on network mounts.");
...
var Volume = class Volume {
	...
	*/
	static async create(name, opts) {
		...
		const res = await new ApiClient(config).api.POST("/volumes", {
			body: { name },
			signal: config.getSignal(opts === null || opts === void 0 ? void 0 : opts.requestTimeoutMs, opts === null || opts === void 0 ? void 0 : opts.signal)
		});
		const err = handleApiError(res, VolumeError);
		if (err) throw err;
		if (!res.data) throw new Error("Response data is missing");
		return new Volume(res.data.volumeID, res.data.name, res.data.token, config.domain, config.debug, config.proxy);
	}
	/**
	...
	*/
	static async connect(volumeId, opts) {
		...
		return new Volume(volumeId, name, token, config.domain, config.debug, config.proxy);
	}
	/**
	...
	*/
	static async getInfo(volumeId, opts) {
		...
		if (res.response.status === 404) throw new NotFoundError(`Volume ${volumeId} not found`);
		const err = handleApiError(res, VolumeError);
		if (err) throw err;
		return {
	...
	*/
	static async list(opts) {
		var _res$data;
		const config = new ConnectionConfig(opts);
		const res = await new ApiClient(config).api.GET("/volumes", { signal: config.getSignal(opts === null || opts === void 0 ? void 0 : opts.requestTimeoutMs, opts === null || opts === void 0 ? void 0 : opts.signal) });
		const err = handleApiError(res, VolumeError);
		if (err) throw err;
	...
	*/
	async list(path, opts) {
		var _this = this;
		const config = new VolumeConnectionConfig(_this, opts);
		const res = await new VolumeApiClient(config).api.GET("/volumecontent/{volumeID}/dir", {
			params: {
				path: { volumeID: _this.volumeId },
				query: {
					path,
		...
		if (res.response.status === 404) throw new NotFoundError(`Path ${path} not found`);
		const err = handleApiError(res, VolumeError);
		if (err) throw err;
	...
	*/
	async makeDir(path, opts) {
		var _this2 = this;
		const config = new VolumeConnectionConfig(_this2, opts);
		const res = await new VolumeApiClient(config).api.POST("/volumecontent/{volumeID}/dir", {
			params: {
				path: { volumeID: _this2.volumeId },
				query: {
					path,
	...
	*/
	async getInfo(path, opts) {
		var _this3 = this;
		const config = new VolumeConnectionConfig(_this3, opts);
		const res = await new VolumeApiClient(config).api.GET("/volumecontent/{volumeID}/path", {
			params: {
				path: { volumeID: _this3.volumeId },
				query: { path }
			},
			signal: config.getSignal()
		});
	...
	*/
	async exists(path, opts) {
		var _this4 = this;
		try {
			await _this4.getInfo(path, opts);
			return true;
		} catch (err) {
	...
	*/
	async updateMetadata(path, metadata, opts) {
		var _this5 = this;
		const config = new VolumeConnectionConfig(_this5, opts);
		const res = await new VolumeApiClient(config).api.PATCH("/volumecontent/{volumeID}/path", {
			params: {
				path: { volumeID: _this5.volumeId },
				query: { path }
			},
			body: {
				uid: metadata.uid,
	...
	async readFile(path, opts) {
		...
		const format = (_opts$format = opts === null || opts === void 0 ? void 0 : opts.format) !== null && _opts$format !== void 0 ? _opts$format : "text";
		const config = new VolumeConnectionConfig(_this6, _objectSpread2(_objectSpread2({}, opts), {}, { requestTimeoutMs: (_opts$requestTimeoutM = opts === null || opts === void 0 ? void 0 : opts.requestTimeoutMs) !== null && _opts$requestTimeoutM !== void 0 ? _opts$requestTimeoutM : FILE_TIMEOUT_MS }));
		...
		const err = handleApiError(res, VolumeError);
		if (err) throw err;
		if (format === "bytes") return res.data instanceof ArrayBuffer ? new Uint8Array(res.data) : /* @__PURE__ */ new Uint8Array();
	...
	*/
	async writeFile(path, data, opts) {
		...
		const config = new VolumeConnectionConfig(_this7, _objectSpread2(_objectSpread2({}, opts), {}, { requestTimeoutMs: (_opts$requestTimeoutM2 = opts === null || opts === void 0 ? void 0 : opts.requestTimeoutMs) !== null && _opts$requestTimeoutM2 !== void 0 ? _opts$requestTimeoutM2 : FILE_TIMEOUT_MS }));
		const client = new VolumeApiClient(config);
		const body = await toUploadBody(data);
		const isStream = body instanceof ReadableStream;
		const signal = isStream ? opts === null || opts === void 0 ? void 0 : opts.signal : config.getSignal();
...
function createEnvdFetchForRuntime(currentRuntime = runtime, options = {}) {
	...
	return (async (input, init) => {
		var _fetcherPromise;
		(_fetcherPromise = fetcherPromise) !== null && _fetcherPromise !== void 0 || (fetcherPromise = buildEnvdFetcher(options));
		return (await fetcherPromise)(input, init);
	});
}
async function buildEnvdFetcher(options) {
	var _options$loadUndici, _options$inflightLimi, _options$connectionLi;
	const undici = await ((_options$loadUndici = options.loadUndici) !== null && _options$loadUndici !== void 0 ? _options$loadUndici : loadUndici)();
	const inflightLimit = (_options$inflightLimi = options.inflightLimit) !== null && _options$inflightLimi !== void 0 ? _options$inflightLimi : 0;
	if (!undici) return limitConcurrency(fetch, inflightLimit);
	const { Agent, ProxyAgent, fetch: undiciFetch } = undici;
	const connections = (_options$connectionLi = options.connectionLimit) !== null && _options$connectionLi !== void 0 ? _options$connectionLi : DEFAULT_ENVD_CONNECTION_LIMIT;
	const dispatcher = options.proxy ? new ProxyAgent({
		uri: options.proxy,
		allowH2: true,
		connections
...
//#endregion
//#region src/envd/process/process_pb.ts
/**
* Describes the file process/process.proto.
*/
const file_process_process = /*@__PURE__*/ (0, _bufbuild_protobuf_codegenv2.fileDesc)("");
/**
* @generated from service process.Process
*/
const Process = /*@__PURE__*/ (0, _bufbuild_protobuf_codegenv2.serviceDesc)(file_process_process, 0);
//#endregion
...
var Pty = class {
	constructor(transport, envdApi, connectionConfig) {
		...
		this.rpc = (0, _connectrpc_connect.createClient)(Process, this.transport);
...
var Commands = class {
	constructor(transport, envdApi, connectionConfig) {
		...
		_defineProperty(this, "envdVersion", void 0);
		_defineProperty(this, "checkHealth", void 0);
		this.rpc = (0, _connectrpc_connect.createClient)(Process, transport);
		this.envdVersion = envdApi.version;
		this.checkHealth = () => checkSandboxHealth(this.envdApi);
	}
	/**
	...
	*/
	get supportsStdinClose() {
		return (0, compare_versions.compareVersions)(this.envdVersion, ENVD_ENVD_CLOSE) >= 0;
	}
	/**
	...
	*/
	...
	async start(cmd, opts) {
		var _this7 = this;
		var _opts$requestTimeoutM2, _opts$timeoutMs2;
		if ((opts === null || opts === void 0 ? void 0 : opts.stdin) === false && (0, compare_versions.compareVersions)(_this7.envdVersion, "0.3.0") < 0) throw new SandboxError(`Sandbox envd version ${_this7.envdVersion} can't specify stdin, it's always turned on. Please rebuild your template if you need this feature.`);
...
function resolveRulesForBody(rules) {
	const out = {};
	for (const [host, hostRules] of rules) out[host] = hostRules.map((rule) => rule.transform === void 0 ? {} : { transform: rule.transform });
	return out;
}
function buildNetworkEgress(network) {
	...
	return _objectSpread2(_objectSpread2(_objectSpread2({}, allowOut !== void 0 ? { allowOut } : {}), denyOut !== void 0 ? { denyOut } : {}), network.rules !== void 0 ? { rules: resolveRulesForBody(rules) } : {});
}
function buildNetworkBody(network) {
	if (!network) return;
	return _objectSpread2(_objectSpread2(_objectSpread2({}, buildNetworkEgress(network)), network.allowPublicTraffic !== void 0 ? { allowPublicTraffic: network.allowPublicTraffic } : {}), network.maskRequestHost !== void 0 ? { maskRequestHost: network.maskRequestHost } : {});
}
function buildNetworkUpdateBody(network) {
	return _objectSpread2(_objectSpread2({}, buildNetworkEgress(network)), network.allowInternetAccess !== void 0 ? { allow_internet_access: network.allowInternetAccess } : {});
}
var SandboxApi = class {
	...
	*/
	static async getMetrics(sandboxId, opts) {
		...
		const start = (opts === null || opts === void 0 ? void 0 : opts.start) ? Math.round(opts.start.getTime() / 1e3) : void 0;
		const end = (opts === null || opts === void 0 ? void 0 : opts.end) ? Math.round(opts.end.getTime() / 1e3) : void 0;
	...
	*/
	static async pause(sandboxId, opts) {
		var _opts$keepMemory, _res$error6, _res$error7;
		const config = new ConnectionConfig(opts);
		const res = await new ApiClient(config).api.POST("/sandboxes/{sandboxID}/pause", {
			params: { path: { sandboxID: sandboxId } },
			body: { memory: (_opts$keepMemory = opts === null || opts === void 0 ? void 0 : opts.keepMemory) !== null && _opts$keepMemory !== void 0 ? _opts$keepMemory : true },
			signal: config.getSignal(opts === null || opts === void 0 ? void 0 : opts.requestTimeoutMs, opts === null || opts === void 0 ? void 0 : opts.signal)
		});
	...
	*/
	static async createSnapshot(sandboxId, opts) {
		...
		const res = await new ApiClient(config).api.POST("/sandboxes/{sandboxID}/snapshots", {
			params: { path: { sandboxID: sandboxId } },
			body: (opts === null || opts === void 0 ? void 0 : opts.name) ? { name: opts.name } : {},
			signal: config.getSignal(opts === null || opts === void 0 ? void 0 : opts.requestTimeoutMs, opts === null || opts === void 0 ? void 0 : opts.signal)
		});
	...
	*/
	...
	static async createSandbox(template, timeoutMs, opts) {
		var _this3 = this;
		var _opts$lifecycle$onTim, _opts$lifecycle, _onTimeout$keepMemory, _opts$lifecycle$autoR, _opts$lifecycle2, _opts$secure, _opts$allowInternetAc;
		const config = new ConnectionConfig(opts);
		const client = new ApiClient(config);
		const onTimeout = (_opts$lifecycle$onTim = opts === null || opts === void 0 || (_opts$lifecycle = opts.lifecycle) === null || _opts$lifecycle === void 0 ? void 0 : _opts$lifecycle.onTimeout) !== null && _opts$lifecycle$onTim !== void 0 ? _opts$lifecycle$onTim : "kill";
		...
		const autoResume = (_opts$lifecycle$autoR = opts === null || opts === void 0 || (_opts$lifecycle2 = opts.lifecycle) === null || _opts$lifecycle2 === void 0 ? void 0 : _opts$lifecycle2.autoResume) !== null && _opts$lifecycle$autoR !== void 0 ? _opts$lifecycle$autoR : false;
		...
		const body = {
			...
			network: buildNetworkBody(opts === null || opts === void 0 ? void 0 : opts.network),
		...
		const err = handleApiError(res);
		if (err) throw err;
		if ((0, compare_versions.compareVersions)(res.data.envdVersion, "0.1.0") < 0) {
			await _this3.kill(res.data.sandboxID, opts);
	...
	static async connectSandbox(sandboxId, opts) {
		var _opts$timeoutMs, _res$error10;
		const timeoutMs = (_opts$timeoutMs = opts === null || opts === void 0 ? void 0 : opts.timeoutMs) !== null && _opts$timeoutMs !== void 0 ? _opts$timeoutMs : DEFAULT_SANDBOX_TIMEOUT_MS;
		const config = new ConnectionConfig(opts);
		...
		if (((_res$error10 = res.error) === null || _res$error10 === void 0 ? void 0 : _res$error10.code) === 404) throw new SandboxNotFoundError(`Paused sandbox ${sandboxId} not found`);
		const err = handleApiError(res);
		if (err) throw err;
		return {
			sandboxId: res.data.sandboxID,
...
var SandboxPaginator = class extends Paginator {
	...
	async nextItems(opts) {
		var _this4 = this;
		var _this$query, _this$query2, _res$data2;
		if (!_this4.hasNext) throw new Error("No more items to fetch");
		let metadata = void 0;
		if ((_this$query = _this4.query) === null || _this$query === void 0 ? void 0 : _this$query.metadata) {
			const encodedPairs = Object.fromEntries(Object.entries(_this4.query.metadata).map(([key, value]) => [encodeURIComponent(key), encodeURIComponent(value)]));
			metadata = new URLSearchParams(encodedPairs).toString();
		}
		const config = new ConnectionConfig(_objectSpread2(_objectSpread2({}, _this4.opts), opts));
		const res = await new ApiClient(config).api.GET("/v2/sandboxes", {
			params: { query: {
				metadata,
				state: (_this$query2 = _this4.query) === null || _this$query2 === void 0 ? void 0 : _this$query2.state,
				limit: _this4.limit,
				nextToken: _this4.nextToken
			} },
			signal: config.getSignal(opts === null || opts === void 0 ? void 0 : opts.requestTimeoutMs, opts === null || opts === void 0 ? void 0 : opts.signal)
		});
		const err = handleApiError(res);
		if (err) throw err;
		_this4.updatePagination(res.response);
		return ((_res$data2 = res.data) !== null && _res$data2 !== void 0 ? _res$data2 : []).map((sandbox) => {
...
var SnapshotPaginator = class extends Paginator {
	...
	async nextItems(opts) {
		var _this5 = this;
		var _res$data3;
		if (!_this5.hasNext) throw new Error("No more items to fetch");
		const config = new ConnectionConfig(_objectSpread2(_objectSpread2({}, _this5.opts), opts));
		const res = await new ApiClient(config).api.GET("/snapshots", {
			params: { query: {
				sandboxID: _this5.sandboxId,
				limit: _this5.limit,
				nextToken: _this5.nextToken
			} },
			signal: config.getSignal(opts === null || opts === void 0 ? void 0 : opts.requestTimeoutMs, opts === null || opts === void 0 ? void 0 : opts.signal)
		});
		const err = handleApiError(res);
		if (err) throw err;
		_this5.updatePagination(res.response);
		return ((_res$data3 = res.data) !== null && _res$data3 !== void 0 ? _res$data3 : []).map((snapshot) => {
			var _snapshot$names;
			return {
...
var Sandbox = class extends SandboxApi {
	...
	*/
	constructor(opts) {
		...
		const rpcTransport = (0, _connectrpc_connect_web.createConnectTransport)({
	...
	*/
	...
	static async create(templateOrOpts, opts) {
		var _this = this;
		var _templateOrOpts$templ, _sandboxOpts$timeoutM;
		...
		const sandbox = new _this(_objectSpread2(_objectSpread2({}, await SandboxApi.createSandbox(template, (_sandboxOpts$timeoutM = sandboxOpts === null || sandboxOpts === void 0 ? void 0 : sandboxOpts.timeoutMs) !== null && _sandboxOpts$timeoutM !== void 0 ? _sandboxOpts$timeoutM : _this.defaultSandboxTimeoutMs, sandboxOpts)), config));
		if (sandboxOpts === null || sandboxOpts === void 0 ? void 0 : sandboxOpts.mcp) {
			var _sandbox$mcpToken;
			sandbox.mcpToken = crypto.randomUUID();
			const res = await sandbox.commands.run(`mcp-gateway --config ${shellQuote(JSON.stringify(sandboxOpts.mcp))}`, {
				user: "root",
	...
	*/
	async connect(opts) {
		var _this3 = this;
		if (_this3.connectionConfig.debug) return _this3;
		await SandboxApi.connectSandbox(_this3.sandboxId, _this3.resolveApiOpts(opts));
		return _this3;
	}
	/**
	...
	*/
	async isRunning(opts) {
		var _this4 = this;
		const signal = _this4.connectionConfig.getSignal(opts === null || opts === void 0 ? void 0 : opts.requestTimeoutMs, opts === null || opts === void 0 ? void 0 : opts.signal);
		const res = await _this4.envdApi.api.GET("/health", { signal });
		if (res.response.status == 502) return false;
	...
	*/
	async setTimeout(timeoutMs, opts) {
		var _this5 = this;
		if (_this5.connectionConfig.debug) return;
		await SandboxApi.setTimeout(_this5.sandboxId, timeoutMs, _this5.resolveApiOpts(opts));
	}
	/**
	...
	*/
	async updateNetwork(network, opts) {
		var _this6 = this;
		await SandboxApi.updateNetwork(_this6.sandboxId, network, _this6.resolveApiOpts(opts));
	}
	/**
	* Kill the sandbox.
	*
	...
	*/
	async kill(opts) {
		var _this7 = this;
		if (_this7.connectionConfig.debug) return true;
		return await SandboxApi.kill(_this7.sandboxId, _this7.resolveApiOpts(opts));
	}
	/**
	...
	*/
	async pause(opts) {
		var _this8 = this;
		return await SandboxApi.pause(_this8.sandboxId, _this8.resolveApiOpts(opts));
	}
	/**
	...
	*/
	async betaPause(opts) {
		var _this9 = this;
		return await SandboxApi.betaPause(_this9.sandboxId, _this9.resolveApiOpts(opts));
	}
	/**
	...
	*/
	async createSnapshot(opts) {
		var _this10 = this;
		return await SandboxApi.createSnapshot(_this10.sandboxId, _objectSpread2(_objectSpread2({}, _this10.resolveApiOpts(opts)), {}, { name: opts === null || opts === void 0 ? void 0 : opts.name }));
	}
	/**
	...
	*/
	async getMcpToken() {
		var _this11 = this;
		if (!_this11.mcpToken) _this11.mcpToken = await _this11.files.read("/etc/mcp-gateway/.token", { user: "root" });
		return _this11.mcpToken;
	}
	/**
	...
	*/
	async uploadUrl(path, opts) {
		...
		const useSignature = !!_this12.envdAccessToken;
		...
		if (username == void 0 && (0, compare_versions.compareVersions)(_this12.envdApi.version, "0.4.0") < 0) username = defaultUsername;
		const filePath = path !== null && path !== void 0 ? path : "";
		const fileUrl = _this12.fileUrl(filePath, username);
		if (useSignature) {
			...
			const sig = await getSignature({
				...
				envdAccessToken: _this12.envdAccessToken
			});
	...
	*/
	async downloadUrl(path, opts) {
		...
		const useSignature = !!_this13.envdAccessToken;
		if (!useSignature && opts.useSignatureExpiration != void 0) throw new InvalidArgumentError("Signature expiration can be used only when sandbox is created as secured.");
		let username = opts.user;
		if (username == void 0 && (0, compare_versions.compareVersions)(_this13.envdApi.version, "0.4.0") < 0) username = defaultUsername;
		const fileUrl = _this13.fileUrl(path, username);
		if (useSignature) {
			const url = new URL(fileUrl);
			const sig = await getSignature({
				path,
				...
				user: username,
				expirationInSeconds: opts.useSignatureExpiration,
				envdAccessToken: _this13.envdAccessToken
			});
			url.searchParams.set("signature", sig.signature);
			if (sig.expiration) url.searchParams.set("signature_expiration", sig.expiration.toString());
			return url.toString();
	...
	*/
	async getInfo(opts) {
		var _this14 = this;
		return await SandboxApi.getInfo(_this14.sandboxId, _this14.resolveApiOpts(opts));
	}
	/**
	* Get the metrics of the sandbox.
	*
	...
	*/
	async getMetrics(opts) {
		var _this15 = this;
		if (_this15.connectionConfig.debug) return [];
		if (_this15.envdApi.version) {
			if ((0, compare_versions.compareVersions)(_this15.envdApi.version, "0.1.5") < 0) throw new TemplateError("You need to update the template to use the new SDK.");
			if ((0, compare_versions.compareVersions)(_this15.envdApi.version, "0.2.4") < 0) {
				var _this$connectionConfi5, _this$connectionConfi6;
				(_this$connectionConfi5 = _this15.connectionConfig.logger) === null || _this$connectionConfi5 === void 0 || (_this$connectionConfi6 = _this$connectionConfi5.warn) === null || _this$connectionConfi6 === void 0 || _this$connectionConfi6.call(_this$connectionConfi5, "Disk metrics are not supported in this version of the sandbox, please rebuild the template to get disk metrics.");
			}
		}
		return await SandboxApi.getMetrics(_this15.sandboxId, _this15.resolveApiOpts(opts));
	}
	resolveApiOpts(opts) {
		return _objectSpread2(_objectSpread2({}, this.connectionConfig), opts !== null && opts !== void 0 ? opts : {});
	}
...
*/
const levels = {
	error: chalk.default.red("ERROR"),
	warn: chalk.default.hex("#FF4400")("WARN "),
	info: chalk.default.hex("#FF8800")("INFO "),
	debug: chalk.default.gray("DEBUG")
};
/**
...
var DefaultBuildLogger = class {
	...
	formatLogLine(line) {
		return `${this.formatTimerLine().padEnd(5)} | ${chalk.default.dim(line.timestamp.toLocaleTimeString(void 0, {
			hour: "2-digit",
			minute: "2-digit",
			second: "2-digit"
...
function validateRelativePath(src, stackTrace) {
	if (node_path.default.isAbsolute(src)) throw new TemplateError(`Invalid source path "${src}": absolute paths are not allowed. Use a relative path within the context directory.`, stackTrace);
	const normalized = node_path.default.normalize(src);
	if (normalized === ".." || normalized.startsWith(".." + node_path.default.sep)) throw new TemplateError(`Invalid source path "${src}": path escapes the context directory. The path must stay within the context directory.`, stackTrace);
}
/**
...
async function calculateFilesHash(src, dest, contextPath, ignorePatterns, resolveSymlinks, stackTrace) {
	...
	for (const file of files) {
		...
		if (file.isSymbolicLink()) {
			const stats = node_fs.default.statSync(file.fullpath(), { throwIfNoEntry: false });
			if (!(resolveSymlinks && ((stats === null || stats === void 0 ? void 0 : stats.isFile()) || (stats === null || stats === void 0 ? void 0 : stats.isDirectory())))) {
				...
				const content = node_fs.default.readlinkSync(file.fullpath());
				hash.update(content);
				continue;
			}
		}
		...
		hashStats(stats);
		if (stats.isFile()) {
			const content = node_fs.default.readFileSync(file.fullpath());
...
*/
async function tarFileStream(fileName, fileContextPath, ignorePatterns, resolveSymlinks, gzip) {
	const { create } = await dynamicImport("tar");
	const { createReadStream } = await dynamicImport("node:fs");
	const filePaths = (await getAllFilesInPath(fileName, fileContextPath, ignorePatterns, true)).map((file) => file.relativePosix());
	const tmpDir = await node_fs.default.promises.mkdtemp(node_path.default.join(node_os.default.tmpdir(), "e2b-template-"));
	const tarPath = node_path.default.join(tmpDir, "context.tar.gz");
	const removeTmpDir = () => node_fs.default.promises.rm(tmpDir, {
		recursive: true,
		force: true
	}).catch(() => {});
	try {
		...
		const { size } = await node_fs.default.promises.stat(tarPath);
...
async function uploadFile(options, stackTrace, abortOpts) {
	...
	try {
		...
		const { Readable } = await dynamicImport("node:stream");
		const tar = await tarFileStream(fileName, fileContextPath, ignorePatterns, resolveSymlinks, gzip);
		uploadStream = tar.stream;
		const res = await fetch(url, {
			method: "PUT",
			body: Readable.toWeb(uploadStream),
			headers: { "Content-Length": tar.size.toString() },
			duplex: "half",
			signal: buildRequestSignal((_abortOpts$requestTim = abortOpts === null || abortOpts === void 0 ? void 0 : abortOpts.requestTimeoutMs) !== null && _abortOpts$requestTim !== void 0 ? _abortOpts$requestTim : FILE_UPLOAD_TIMEOUT_MS, abortOpts === null || abortOpts === void 0 ? void 0 : abortOpts.signal)
		});
		if (!res.ok) throw new FileUploadError(`Failed to upload file: ${res.statusText}`, stackTrace);
	} catch (error) {
		uploadStream === null || uploadStream === void 0 || uploadStream.destroy();
...
function parseDockerfile(dockerfileContentOrPath, templateBuilder) {
	...
	const instructions = dockerfile_ast.DockerfileParser.parse(dockerfileContent).getInstructions();
...
*/
var TemplateBase = class TemplateBase {
	constructor(options) {
		...
		_defineProperty(this, "stackTracesEnabled", true);
		_defineProperty(this, "stackTracesOverride", void 0);
		this.fileContextPath = (_options$fileContextP = options === null || options === void 0 ? void 0 : options.fileContextPath) !== null && _options$fileContextP !== void 0 ? _options$fileContextP : runtime === "browser" ? "." : (_getCallerDirectory = getCallerDirectory(3)) !== null && _getCallerDirectory !== void 0 ? _getCallerDirectory : ".";
	...
	*/
	...
	static async build(template, nameOrOptions, options) {
		...
		try {
			var _buildOptions$onBuild, _buildOptions$onBuild2;
			(_buildOptions$onBuild = buildOptions.onBuildLogs) === null || _buildOptions$onBuild === void 0 || _buildOptions$onBuild.call(buildOptions, new LogEntryStart(/* @__PURE__ */ new Date(), "Build started"));
			const baseTemplate = template;
			const config = new ConnectionConfig(buildOptions);
			const client = new ApiClient(config);
			const data = await baseTemplate.build(client, config, name, buildOptions);
			(_buildOptions$onBuild2 = buildOptions.onBuildLogs) === null || _buildOptions$onBuild2 === void 0 || _buildOptions$onBuild2.call(buildOptions, new LogEntry(/* @__PURE__ */ new Date(), "info", "Waiting for logs..."));
			...
			var _buildOptions$onBuild3;
			(_buildOptions$onBuild3 = buildOptions.onBuildLogs) === null || _buildOptions$onBuild3 === void 0 || _buildOptions$onBuild3.call(buildOptions, new LogEntryEnd(/* @__PURE__ */ new Date(), "Build finished"));
		}
	}
	...
	*/
	...
	fromImage(baseImage, credentials) {
		if (credentials && (!credentials.username || !credentials.password)) throw new InvalidArgumentError("Both username and password are required when providing registry credentials", getCallerFrame(2));
	...
	fromDockerfile(dockerfileContentOrPath) {
		const { baseImage } = this.runInStackTraceOverrideContext(() => parseDockerfile(dockerfileContentOrPath, this), getCallerFrame(2));
		this.baseImage = baseImage;
		this.baseTemplate = void 0;
		if (this.forceNextLayer) this.force = true;
		this.collectStackTrace();
	...
	copy(src, dest, options) {
		...
		const stackTrace = getCallerFrame(2);
	...
	remove(path, options) {
		...
		return this.runInNewStackTraceContext(() => this.runCmd(args.join(" "), { user: options === null || options === void 0 ? void 0 : options.user }));
	}
	...
	npmInstall(packages, options) {
		...
		if (packageList) args.push(...packageList);
		return this.runInNewStackTraceContext(() => this.runCmd(args.join(" "), { user: (options === null || options === void 0 ? void 0 : options.g) ? "root" : void 0 }));
	}
	...
	aptInstall(packages, options) {
		...
		return this.runInNewStackTraceContext(() => this.runCmd(["apt-get update", `DEBIAN_FRONTEND=noninteractive DEBCONF_NOWARNINGS=yes apt-get install -y ${(options === null || options === void 0 ? void 0 : options.noInstallRecommends) ? "--no-install-recommends " : ""}${(options === null || options === void 0 ? void 0 : options.fixMissing) ? "--fix-missing " : ""}${packageList.join(" ")}`], { user: "root" }));
	}
	addMcpServer(servers) {
		if (this.baseTemplate !== "mcp-gateway") throw new BuildError("MCP servers can only be added to mcp-gateway template", getCallerFrame(2));
	...
	betaDevContainerPrebuild(devcontainerDirectory) {
		if (this.baseTemplate !== "devcontainer") throw new BuildError("Devcontainers can only used in the devcontainer template", getCallerFrame(2));
	...
	*/
	collectStackTrace(stackTracesDepth = 3) {
	...
	*/
	async toJSON(computeHashes) {
		var _this = this;
		let instructions = _this.instructions;
		if (computeHashes) instructions = await _this.instructionsWithHashes();
		return JSON.stringify(_this.serialize(instructions), void 0, 2);
	}
	/**
	...
	*/
	async build(client, config, name, options) {
		...
		if (options.skipCache) _this2.force = true;
		...
		const instructionsWithHashes = await _this2.instructionsWithHashes();
		const uploadPromises = instructionsWithHashes.map(async (instruction, index) => {
			...
			if (index + 1 >= 0 && index + 1 < _this2.stackTraces.length) stackTrace = _this2.stackTraces[index + 1];
			...
			if (forceUpload && url != null || present === false && url != null) {
				...
				await uploadFile({
					fileName: src,
					fileContextPath: _this2.fileContextPath.toString(),
					url,
					ignorePatterns: [..._this2.fileIgnorePatterns, ...readDockerignore(_this2.fileContextPath.toString())],
		...
		await triggerBuild(client, {
			templateID,
			buildID,
			template: _this2.serialize(instructionsWithHashes)
		}, config.getSignal(void 0, options.signal));
		return {
			alias: name,
			name,
	...
	*/
	async instructionsWithHashes() {
		var _this3 = this;
		return Promise.all(_this3.instructions.map(async (instruction, index) => {
			...
			let stackTrace = void 0;
			if (index + 1 >= 0 && index + 1 < _this3.stackTraces.length) stackTrace = _this3.stackTraces[index + 1];
			return _objectSpread2(_objectSpread2({}, instruction), {}, { filesHash: await calculateFilesHash(src, dest, _this3.fileContextPath.toString(), [..._this3.fileIgnorePatterns, ...runtime === "browser" ? [] : readDockerignore(_this3.fileContextPath.toString())], (_instruction$resolveS2 = instruction.resolveSymlinks) !== null && _instruction$resolveS2 !== void 0 ? _instruction$resolveS2 : false, stackTrace) });
		}));
	}
	/**
...
//#endregion
exports.ALL_TRAFFIC = ALL_TRAFFIC;
...
exports.GitUpstreamError = GitUpstreamError;
exports.InvalidArgumentError = InvalidArgumentError;
exports.LogEntry = LogEntry;
exports.LogEntryEnd = LogEntryEnd;
exports.LogEntryStart = LogEntryStart;
exports.NotEnoughSpaceError = NotEnoughSpaceError;
exports.NotFoundError = NotFoundError;
exports.RateLimitError = RateLimitError;
exports.ReadyCmd = ReadyCmd;
...
exports.VolumeFileType = VolumeFileType;
exports.default = src_default;
exports.defaultBuildLogger = defaultBuildLogger;
