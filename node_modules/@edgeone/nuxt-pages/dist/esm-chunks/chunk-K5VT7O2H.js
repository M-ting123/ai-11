...
var copyNuxtServerCode = async (ctx) => {
  ...
  try {
    ...
    if (isError(error) && error.code === "ENOENT") {
      ctx.failBuild(
        `Failed creating server handler. Nuxt server directory not found at expected location "${nuxtServerDir}". Make sure you have built your Nuxt application.`
      );
    } else {
      throw error;
    }
  ...
  const srcDir = nuxtServerDir;
  const destDir = ctx.serverHandlerDir;
...
var verifyNuxtHandlerDirStructure = async (ctx) => {
  const expectedServerPath = ctx.serverHandlerDir;
  if (!existsSync(expectedServerPath)) {
    ctx.failBuild(
      `Failed creating server handler. Server handler directory not found at expected location "${expectedServerPath}".`
    );
  }
  const serverFiles = await readdir(expectedServerPath);
  const hasServerEntry = serverFiles.some(
    (file) => file === "index.mjs" || file === "index.js" || file.startsWith("index.")
  );
  if (!hasServerEntry) {
    ctx.failBuild(
      `Failed creating server handler. No server entry point found in "${expectedServerPath}".`
    );
  }
};
