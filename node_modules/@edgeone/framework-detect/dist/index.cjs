...
var pathModule__namespace = /*#__PURE__*/_interopNamespaceDefault(pathModule);
...
const Frameworks = [
    {
        ...
        detectors: {
            some: [
                {
                    path: 'next.config.ts',
                    matchAttribute: { key: 'output', value: 'export' },
                },
                {
                    path: 'next.config.js',
                    matchAttribute: { key: 'output', value: 'export' },
...
var hasOwn$3 = Object.hasOwn || (function (obj, propName) { return (
...
function getOptions(opts) {
  ...
  for (var opt in defaultOptions)
    { options[opt] = opts && hasOwn$3(opts, opt) ? opts[opt] : defaultOptions[opt]; }
...
pp$8.parseWithClause = function() {
  ...
  while (!this.eat(types$1.braceR)) {
    ...
    if (hasOwn$3(attributeKeys, keyName))
...
pp$7.checkLValSimple = function(expr, bindingType, checkClashes) {
  ...
  case "Identifier":
    ...
    if (isBind) {
      ...
      if (checkClashes) {
        if (hasOwn$3(checkClashes, expr.name))
...
pp$1.regexp_validateUnicodePropertyNameAndValue = function(state, name, value) {
  if (!hasOwn$3(state.unicodeProperties.nonBinary, name))
    { state.raise("Invalid property name"); }
...
async function matchesLocal({ name, detectors, info, dirFiles, pkgStr, getFileContent, }) {
    ...
    if (!result.every((res) => !!res))
        return undefined;
    return result[0];
    async function check({ path, matchContent, matchPackage, matchAttribute, }) {
        if (matchPackage && matchContent) {
            throw new Error(`Cannot specify "matchPackage" and "matchContent" in the same detector for "${name}"`);
        }
        if (matchPackage && matchAttribute) {
            throw new Error(`Cannot specify "matchPackage" and "matchAttribute" in the same detector for "${name}"`);
        }
                ...
                simple(ast, {
                    Property(node) {
                        if (node.type === 'Property'
                            && node.key?.name === matchAttribute.key
                            && node.value?.value === matchAttribute.value) {
                            match = true;
                        }
                    }
                });
...
async function describeFramework(projectRoot) {
    ...
    let dirFiles = [];
    try {
        dirFiles = fs__namespace.readdirSync(projectRoot).map((name) => ({ name }));
    }
    catch {
    ...
    let pkgStr = "";
    try {
        const pkgPath = pathModule__namespace.join(projectRoot, "package.json");
        pkgStr = fs__namespace.readFileSync(pkgPath, "utf-8");
    }
    catch {
    ...
    async function getFileContent(filePath) {
        try {
            return fs__namespace.readFileSync(pathModule__namespace.join(projectRoot, filePath), "utf-8");
        }
        catch {
            return null;
        }
    ...
    for (const framework of Frameworks) {
        ...
        if (frameworkInfo !== undefined) {
            ...
            if (fs__namespace.existsSync(pathModule__namespace.join(projectRoot, "yarn.lock"))) {
                result.BuildCmd = "yarn build";
                result.InstallCmd = "yarn install";
            }
            if (fs__namespace.existsSync(pathModule__namespace.join(projectRoot, "pnpm-lock.yaml"))) {
                result.BuildCmd = "pnpm run build";
                result.InstallCmd = "pnpm install";
            }
            if (fs__namespace.existsSync(pathModule__namespace.join(projectRoot, "bun.lockb"))) {
 ...
 * 匹配框架
 * @param name 框架名称
 * @param detectors 检测器，every 表示所有检测器都通过，some 表示至少一个检测器通过
 * @param info 框架信息
 * @param body 请求体
 * @param pkgStr package.json 内容
 * @param rootDir 项目的根目录
 * @param retDir 返回的项目目录
 * @param octokit octokit 实例
 * @returns 如果匹配到的框架，则返回对应框架信息，否则返回 undefined
 */
async function matchesRemote({ name, detectors, info, body, pkgStr, rootDir, retDir, octokit, user, }) {
    if (!detectors) {
        return;
    }
    const { every, some } = detectors;
    ...
    async function check({ path, matchContent, matchPackage, matchAttribute, }) {
        ...
        let content = pkgStr;
        if (path) {
            // 如果 path 不为空，则检测别的文件，比如 vite.config.js
            if (retDir.data.some((f) => f.name === path) === false)
                return;
            let retPackage = null;
            if (body.Provider === "Github") {
                retPackage = await getGitHubContents(octokit, body, rootDir, path);
            }
            if (body.Provider === "Gitee") {
                retPackage = await getGiteeContents(user, body, rootDir, path);
            }
            if (retPackage === null)
                return;
            content = Buffer.from(retPackage.data.content, "base64").toString();
        }
        if (matchContent) {
            const regex = new RegExp(matchContent, "m");
            const match = content.match(regex);
...
/**
 * 获取 GitHub 仓库内容中指定目录或文件的内容
 * @param octokit octokit 实例
 * @param namespace 仓库所属 namespace
 * @param repoName 仓库名称
 * @param rootDir 项目的根目录，如果为空，则表示获取仓库根目录
 * @param path 文件路径，如果为空，则表示获取仓库根目录
 */
async function getGitHubContents(octokit, body, rootDir, path) {
    return await octokit
        .request("GET /repos/{owner}/{repo}/contents/{path}", {
        owner: body.Namespace,
        repo: body.RepoName,
        path: rootDir ? (path ? rootDir + "/" + path : rootDir) : path,
        headers: {
...
/**
 * 获取 Gitee 仓库内容中指定目录或文件的内容
 * @param httpService http 服务
 * @param body 请求体
 * @param user 用户
 * @param rootDir 项目的根目录，如果为空，则表示获取仓库根目录
 * @param path 文件路径，如果为空，则表示获取仓库根目录
 */
async function getGiteeContents(user, body, rootDir, path) {
    const segment = rootDir ? (path ? rootDir + "/" + path : rootDir) : path;
    try {
        const response = await axios$1.get(`https://gitee.com/api/v5/repos/${body.Namespace}/${body.RepoName}/contents/${segment}`, {
            params: {
...
async function describeFrameworkRemote(body, octokit, user) {
    // 获取带检测项目的根目录
    const rootDir = body.RootDir?.replace(/^(\.|\/)*/, "").replace(/\/*$/, "");
    const retDir = body.Provider === "Github"
        ? await getGitHubContents(octokit, body, rootDir, "")
        : await getGiteeContents(user, body, rootDir, "");
    if (!retDir) {
        return getOtherFramework();
    }
    // 判断是否存在 package.json
    const hasPackageJson = retDir.data.some((f) => f.name === "package.json");
    if (!hasPackageJson) {
        return getOtherFramework();
    }
    // 获取 package.json
    const packageJson = body.Provider === "Github" ? await getGitHubContents(octokit, body, rootDir, "package.json") : await getGiteeContents(user, body, rootDir, "package.json");
    if (!packageJson) {
        return getOtherFramework();
    }
    // 解析 package.json
    let pkgStr;
    try {
        pkgStr = Buffer.from(packageJson.data.content, "base64").toString();
    }
    catch {
        pkgStr = JSON.stringify({
            dependencies: {},
            devDependencies: {},
        });
    }
    // 逐个匹配框架
    for (const framework of Frameworks) {
        const frameworkInfo = await matchesRemote({
            ...framework,
            body,
            pkgStr,
            rootDir,
            retDir,
            octokit: body.Provider === "Github" ? octokit : null,
            user: body.Provider === "Gitee" ? user : null,
        });
        if (frameworkInfo !== undefined) {
            const result = {
                ...frameworkInfo,
            ...
            if (retDir.data.some((f) => f.name === "pnpm-lock.yaml")) {
                result.InstallCmd = "pnpm install";
                result.BuildCmd = "pnpm run build";
            }
            if (retDir.data.some((f) => f.name === "yarn.lock")) {
                result.InstallCmd = "yarn install";
                result.BuildCmd = "yarn build";
            }
            if (retDir.data.some((f) => f.name === "bun.lockb")) {
                result.InstallCmd = "bun install";
                result.BuildCmd = "bun run build";
            }
            return result;
...
exports.describeFrameworkRemote = describeFrameworkRemote;
