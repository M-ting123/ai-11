...
// src/sandbox.ts
import { Sandbox as BaseSandbox, InvalidArgumentError, TimeoutError as TimeoutError3 } from "e2b";
...
var CONNECTION_CLOSED_CODES = ["ECONNRESET", "EPIPE", "UND_ERR_SOCKET"];
...
var Sandbox = class extends BaseSandbox {
  ...
  async handleRequestError(error) {
    if (isConnectionClosedError(error) && // If the state check itself fails we can't tell whether the sandbox
    ...
    await this.isRunning().catch(() => true) === false) {
      return new TimeoutError3(
        "The sandbox was killed while the request was in progress. This can happen when the sandbox times out or is killed manually. You can modify the sandbox timeout by passing 'timeoutMs' when starting the sandbox or calling '.setTimeout' on the sandbox with the desired timeout."
      );
    }
