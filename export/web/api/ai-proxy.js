 ...
 * 环境变量：DEEPSEEK_API_KEY — DeepSeek API 密钥（在 Vercel Dashboard 配置）
 *
 * 支持模块：
 *   - witness_chat    ：AI 证人对话
 *   - ai_assistant    ：AI 助手案件分析
 *   - court_opening   ：AI 生成法官开庭词
 */
...
function errorJson(res, code, message, status = 500) {
  return json(res, { module: "error", error: { code, message } }, status);
}
...
export default async function handler(req, res) {
  ...
  // 仅接受 POST
  if (req.method !== "POST") {
    return errorJson(res, "METHOD_NOT_ALLOWED", "仅支持 POST 请求", 405);
  }
  ...
  try {
    ...
    return errorJson(res, "INVALID_JSON", "请求体不是有效的 JSON", 400);
  }

  // 验证 module 字段
  const module = body.module;
  if (!module) {
    return errorJson(res, "MISSING_MODULE", "请求体缺少 module 字段", 400);
  }
  ...
  if (!apiKey) {
    ...
    return errorJson(res, "NO_API_KEY", "服务端未配置 DEEPSEEK_API_KEY 环境变量", 500);
  }
  ...
  try {
    ...
    switch (module) {
      ...
      case "court_opening":
        ...
        break;
      default:
        return errorJson(res, "UNKNOWN_MODULE", `未知模块: ${module}`, 400);
    }
    return json(res, result);
  } catch (err) {
    console.error(`[ai-proxy] 错误: ${err.message}`);
    return errorJson(res, "INTERNAL_ERROR", err.message, 500);
  }
}
